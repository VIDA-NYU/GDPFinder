...
class M2M(object):
    ...
    def sendRequest(self, endpoint, data={}, max_retries=10):
        ...
        if self.apiKey == None:
            ...
            response = retry_connect(
                url, json_data, headers=headers, max_retries=max_retries
            )
        if response == None:
        ...
        if status != 200:
            if isinstance(output, dict):
                msg = "{} - {} - {}".format(
                    status, output["errorCode"], output["errorMessage"]
                )
            else:
            ...
            if isinstance(output, dict):
                if (
                    output["data"] is None
                    and output["errorCode"] is not None
                    and endpoint != "logout"
                ):
        ...
        if scenes["totalHits"] > scenes["recordsReturned"]:
            logging.warning(
                "M2M.searchScenes - more hits {} than returned records {}, consider increasing maxResults parameter.".format(
                    scenes["totalHits"], scenes["recordsReturned"]
                )
            )
        return scenes
    ...
    def downloadOptions(self, datasetName, entityIds, filterOptions={}):
        ...
        params = {"datasetName": datasetName, "entityIds": entityIds}
        ...
        if requestedDownloadsCount:
            ...
            if (
                ...
                for label in labels:
                    ...
                    downloadUpdate = (
                        requestResultsUpdated["available"]
                        + requestResultsUpdated["requested"]
                    )
                    download_scenes(downloadUpdate, downloadMeta, download_dir)
                ...
                while len(downloadIds) < requestedDownloadsCount:
                    ...
                    logging.info(
                        "M2M.retrieveScenes - {} downloads are not available. Waiting 10 seconds...".format(
                            preparingDownloads
                        )
                    )
                    time.sleep(10)
                ...
                download_scenes(
                    requestResults["availableDownloads"], downloadMeta, download_dir
                )
            return downloadMeta
        else:
...
def retry_connect(url, json_data, headers={}, max_retries=10, sleep_seconds=1):
    retries = 0
    while retries < max_retries:
        try:
            response = requests.post(url, json_data, headers=headers, timeout=10)
        except:
            retries += 1
            logging.info(
                "Connection Timeout - retry number {} of {}".format(
                    retries, max_retries
                )
            )
